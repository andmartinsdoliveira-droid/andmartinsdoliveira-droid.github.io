...
class ShoppingCart {
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div id="checkoutStep2" class="checkout-step" style="display:none;">
            ...
            <div id="payment-brick-container" style="margin-top: 1.5rem;"></div>
  ...
  if (elTel) {
    elTel.textContent =
      this.customerData?.phone && this.customerData.phone.trim()
  ...
  async initMercadoPagoBrick() {
    if (typeof MercadoPago === 'undefined') {
      this.showNotification('Erro: SDK do Mercado Pago não carregado.', 'error');
      return;
    }

    // Limpa o container antes de renderizar para evitar duplicatas ou erros
    const container = document.getElementById('payment-brick-container');
    if (container) container.innerHTML = '';

    if (window.mpTransparente) {
        window.mpTransparente.unmount();
    }

    const publicKey = window.MP_PUBLIC_KEY || 'APP_USR-your-public-key';
    window.mpTransparente = new MercadoPagoTransparente(publicKey);

    const amount = this.getTotalValue();
    ...
    try {
      await window.mpTransparente.renderPaymentBrick(
        'payment-brick-container',
        amount,
        items,
        this.customerData,
        {
          webhookUrl: CartConfig.MAKE_WEBHOOK_URL,
          onSuccess: (result) => {
            this.items = [];
            this.saveToStorage();
            this.updateCounter();
            ...
            setTimeout(() => {
              window.location.href = 'sucesso.html?payment_id=' + (result.id || '');
            }, 2000);
          },
          onError: (error) => {
            this.showNotification('Erro ao processar pagamento. Verifique os dados e tente novamente.', 'error');
          }
        }
      );
    } catch (error) {
