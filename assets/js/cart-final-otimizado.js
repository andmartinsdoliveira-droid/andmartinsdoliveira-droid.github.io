...
class ShoppingCart {
  ...
  renderCartItems() {
    ...
    // Adiciona a mensagem de redirecionamento e entrega se houver itens
    this.renderDeliveryMessage(hasItems);
  }

  renderDeliveryMessage(hasItems) {
    const modalFooter = document.querySelector('#carrinhoModal .modal-footer > div');
    if (!modalFooter) return;

    // Remove mensagem anterior se existir para não duplicar
    const existingMsg = document.getElementById('deliveryInfoMsg');
    if (existingMsg) existingMsg.remove();

    if (hasItems) {
      const msgDiv = document.createElement('div');
      msgDiv.id = 'deliveryInfoMsg';
      msgDiv.style.cssText = 'background-color: #f8f9fa; border-radius: 8px; padding: 1rem; margin-bottom: 1.5rem; border-left: 4px solid #4A90E2; font-size: 0.9rem; color: #333; line-height: 1.4; text-align: left;';
      msgDiv.innerHTML = `
        <p style="margin-bottom: 0.5rem;">🛡️ <strong>Ambiente Seguro:</strong> Você será redirecionado ao <strong>PagBank</strong> para concluir o pagamento com total segurança.</p>
        <p>📧 <strong>Entrega Automática:</strong> Após a confirmação, você receberá o material imediatamente no seu e-mail. <span style="color: #4A90E2; font-weight: 500;">Caso não encontre na Caixa de Entrada, verifique sua pasta de Spam ou Lixo Eletrônico.</span></p>
      `;
      
      // Insere a mensagem antes dos botões de ação
      const actionButtons = modalFooter.querySelector('div[style*="display:flex"]');
      if (actionButtons) {
        modalFooter.insertBefore(msgDiv, actionButtons);
      } else {
        modalFooter.appendChild(msgDiv);
      }
    }
  }
...
cartStyles.textContent = `
  ...
  animation: slideInRight 0.3s ease; /* <-- INCLUSÃO */
  cursor: pointer;
 }
